...
def Hub_vi (input_vi,fused_result):
    huber_loss = HuberLoss()
    Hub_vi=huber_loss(input_vi,fused_result)

    return Hub_vi

def Hub_ir (input_ir,fused_result):
    huber_loss = HuberLoss()
    Hub_ir=huber_loss(input_ir,fused_result)
...
class HuberLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        loss = torch.where(condition, 0.5 * self.mse(y_true, y_pred), self.delta * (self.l1(y_true, y_pred) - 0.5 * self.delta))
