...
class Conv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        if kernel_size == 1:
            ...
            ws = kernel_size
            self.avg_pool = nn.AdaptiveAvgPool2d((ws,ws))#自适应平均池化,经过这个的结果输出大小为(wsxws)

            self.num_lat = int((kernel_size * kernel_size) / 2 + 1)#得到的是一个数这个数是卷积核尺寸的一半+1

            self.ce = nn.Linear(ws*ws, self.num_lat, False)    #输入大小，输出大小，主要用于降维
            self.ce_bn = nn.BatchNorm1d(in_channels)#BN层
            self.ci_bn2 = nn.BatchNorm1d(in_channels)

            self.act = nn.ReLU(inplace=True)#relu
            

            if in_channels // 16:#是否能够被16整除
                self.g = 16
            else:
                self.g = in_channels
            ...
            self.unfold = nn.Unfold(kernel_size, dilation, padding, stride)#展开

            self.sig = nn.Sigmoid()
    def forward(self, x):

        if self.ind:#如果是1*1的大小就是正常的卷积
            ...
            gl = self.avg_pool(x).view(b,c,-1)#变成bxcx(展平)
