...
class Block_ViT(nn.Module):
    def __init__(self, vis, channel_num):
        ...
        self.attn_norm = LayerNorm(32, eps=1e-6)
...
class ChannelTransformer(nn.Module):
    def __init__(self, vis=False, img_size=128, channel_num=[16, 16], patchSize=[128, 128]):
        super().__init__()
        ...
        # self.embeddings_2 = Channel_Embeddings(config,self.patchSize_2, img_size=img_size, in_channels=channel_num[1])
        ...
        # self.reconstruct_1 = Reconstruct(channel_num[0], channel_num[0], kernel_size=1,scale_factor=(1, 1))
        # self.reconstruct_2 = Reconstruct(channel_num[1], channel_num[1], kernel_size=1,scale_factor=(1, 1))
    ...
    def forward(self,en1,en2):
        ...
        channel_filter = self.channel_branch(out)
        spatial_filter = self.spatial_branch(out)
        XCout = out * channel_filter
        XSout = out * spatial_filter
        final_out = XSout + XCout + out
...
# net = ChannelTransformer().to(device)
# # print(net)
# summary(net, [(16,128,128),(16,128,128)])