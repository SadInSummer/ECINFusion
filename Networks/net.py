...
from Networks.CTrans import ChannelTransformer
...
class Convlutioanl_out_relu(nn.Module):
    def __init__(self,  in_channel, out_channel):
        super(Convlutioanl_out_relu, self).__init__()
        # self.padding=(2,2,2,2)
...
class ACTBlock(nn.Module):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=2, out_channel=16,
    ...
    def forward(self, Input):
        ...
        encode_size = (Input.shape[2], Input.shape[3])
        Transformer = self.basicLayer(Input, encode_size)
        out = Transformer+layer

        return out


class AMTSBlock(nn.Module):
    def __init__(self, img_size,patch_size,embed_dim,num_heads, window_size,in_channel, out_channel,
                 mlp_ratio, qkv_bias, qk_scale, drop, attn_drop, patch_norm,depth,downsample,
                 drop_path,  alpha,depth_p, norm_layer,use_checkpoint, bottleneck):
        super(AMTSBlock, self).__init__()
        self.ACTblock = ACTBlock(img_size=img_size,
                                 patch_size=patch_size,
                                 embed_dim=embed_dim,
                                 num_heads=num_heads,
                                 window_size=window_size,
    ...
    def forward(self, input):
        ...
        # convInput = self.convInput(input)
        layer1_out = self.ACTblock(input)
        layer2_out = self.ACTblock(self.ACTblock(input))
        layer3_out = self.ACTblock (self.ACTblock(self.ACTblock(input)))


        out=layer1_out + layer2_out+layer3_out+input
...
class Single_MODEL(nn.Module):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=1, out_channel=16,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., patch_norm=True,depth=1,downsample=None,
                 drop_path=0.,  alpha=300,depth_p=32, norm_layer=nn.LayerNorm,use_checkpoint=False, bottleneck=True ):
        super(Single_MODEL, self).__init__()

        self.convInput= Basic3x3(in_channel, out_channel)
        self.amtsblock = AMTSBlock(img_size=img_size,
                                 patch_size=patch_size,
                                 embed_dim=embed_dim,
                                 num_heads=num_heads,
                                 window_size=window_size,
    ...
    def forward(self, input):
        ...
        convInput = self.convInput(input)
        out = self.amtsblock(convInput)
        # out = self.amtsblock(out)
        # out = self.amtsblock(out)
...
class Final_MODEL(nn.Module):
    def __init__(self,in_channel=1, out_channel=16):
        ...
        self.feature_extracter1 = Single_MODEL()
        self.feature_extracter2 = Single_MODEL()
        self.fusion_maker = ChannelTransformer()
        self.convolutional_out1 = Convlutioanl_out(64,32)
        self.convolutional_out2 = Convlutioanl_out(32,16)
        self.convolutional_out3 = Convlutioanl_out(16,1)
        # self.convolutional_out4 = Convlutioanl_out(8,1)




    def forward(self, input1,input2):
        out1 = self.feature_extracter1(input1)
        out2 = self.feature_extracter2(input2)
        out = self.fusion_maker(out1,out2)
        out = torch.cat([out,out1,out2],dim=1)
        out = self.convolutional_out1(out)
        out = self.convolutional_out2(out)
        out = self.convolutional_out3(out)
        # out = self.convolutional_out4(out)
...
# img=torch.rand((1,2,256,256)).to(device)
